...
public class VMultiselectCalendarPanel extends FocusableFlexTable implements KeyDownHandler, KeyPressHandler,
		MouseOutHandler, MouseDownHandler, MouseUpHandler, BlurHandler, FocusHandler, SubPartAware {
	...
	private class VEventButton extends Button {
		public VEventButton() {
			addMouseDownHandler(VMultiselectCalendarPanel.this);
			addMouseOutHandler(VMultiselectCalendarPanel.this);
			addMouseUpHandler(VMultiselectCalendarPanel.this);
		}
	}
	...
	private ClickHandler dayClickHandler = new ClickHandler() {
		...
		public void onClick(ClickEvent event) {
			...
			if (newDate.getMonth() != displayedMonth.getMonth() || newDate.getYear() != displayedMonth.getYear()) {
				// If an off-month date was clicked, we must change the
				// displayed month and re-render the calendar (#8931)
				changeDisplayedMonth(newDate.getYear(), newDate.getMonth());
			}
			focusDay(newDate);
			selectFocused();
			onSubmit();
	...
	private VMultiselectDateField parent;

	private boolean initialRenderDone = false;

	public VMultiselectCalendarPanel() {
	...
	public void setParentField(VMultiselectDateField parent) {
		this.parent = parent;
	}
		...
		for (int i = 0; i < rowCount; i++) {
			...
			for (int j = 0; j < cellCount; j++) {
				...
				if (widget != null && widget instanceof Day) {
					Day curday = (Day) widget;
					if (curday.getDate().equals(date)) {
						if (multiselecteds.contains(date.getTime())) {
							multiselecteds.remove(date.getTime());
							curday.removeStyleDependentName(CN_SELECTED);

						} else {
							multiselecteds.add(date.getTime());
	...
	private void buildCalendarHeader(boolean needsMonth) {
		...
		setHTML(0, 2, "<span class=\"" + parent.getStylePrimaryName() + "-calendarpanel-month\">" + monthName + " "
				+ year + "</span>");
	}
	...
	private void buildCalendarBody() {
		...
		for (int weekOfMonth = 1; weekOfMonth < 7; weekOfMonth++) {
			for (int dayOfWeek = 0; dayOfWeek < 7; dayOfWeek++) {
				...
				if (multiselecteds != null && multiselecteds.contains(dayDate.getTime())) {
	...
	private void focusNextDay(int days) {
		...
		if (focusedDate.getMonth() == oldMonth && focusedDate.getYear() == oldYear) {
			...
			changeDisplayedMonth(focusedDate.getYear(), focusedDate.getMonth());
		}
	}
	...
	public void focusNextMonth() {
		...
		changeDisplayedMonth(null, displayedMonth.getMonth() + 1);
	}
	 ...
	 */
	public void focusPreviousMonth() {
	 ...
	 */
	public void focusPreviousYear(int years) {
		...
		if (!isDateInsideRange(previousYearDate, Resolution.DAY)) {
			...
			changeDisplayedMonth(previousYearDate.getYear(), previousYearDate.getMonth());
		} else {
			...
			changeDisplayedMonth(displayedMonth.getYear() - years, null);
		}
	}
	 ...
	 */
	public void focusNextYear(int years) {
		...
		if (!isDateInsideRange(nextYearDate, Resolution.DAY)) {
			...
			changeDisplayedMonth(nextYearDate.getYear(), nextYearDate.getMonth());
		} else {
	...
	public void onBlur(final BlurEvent event) {
		if (event.getSource() instanceof VMultiselectCalendarPanel) {
			hasFocus = false;
			focusDay(null);
		}
	}
	...
	private boolean resendChange = true;
	...
	public Set<Long> getMultiselecteds() {
		return multiselecteds;
	}
	...
	public void setDisplayedMonth(Integer year, Integer month) {
		if (focusedDate == null) {
			Date now = new Date();
			// focusedDate must have zero hours, mins, secs, millisecs
			focusedDate = new Date(year == null ? now.getYear() : year, month == null ? now.getMonth() : month,
					now.getDate());
			// TODO: probablemente haya que tener en cuenta que el ultimo dia de
			// mes no coincide con el mes que se esta cambiando (30feb por.ej):
			/*
			 * while (date.getMonth() != requestedMonth) {
			 * date.setDate(date.getDate() - 1); }
			 */
		...
		if (year != null) {
			displayedMonth.setYear(year);
		}
		if (month != null) {
			displayedMonth.setMonth(month);
		}
	}

	private void changeDisplayedMonth(Integer year, Integer month) {
		if (displayedMonth != null && resendChange) {
			Date currentDate = new Date(year == null ? displayedMonth.getYear() : year, month == null ? displayedMonth.getMonth() : month, 1);
			int command = 0;
			long diffInMillies = currentDate.getTime() - displayedMonth.getTime();
			...
			if (command != 0) {
				parent.displayedMonthChanged(this, command);
			}
		}
	...
	public void setResendChange(boolean resendChange) {
		this.resendChange = resendChange;
	}
}
